...
int main(void)
{
	...
	while (!(old_rec.id == SENTINEL || trans_rec.id == SENTINEL))
	{
		...
		else if (old_rec.id == trans_rec.id) // 올드레코드의 id와 트랜젝션의 id가 같을때
		{
			switch (trans_rec.update_code)
			{
			case 'A': // 추가할 경우
				Error_Print('A'); // 에러코드 출력
				Get_Transaction(); // 새 트랜젝션 가져오기
				break;
			case 'C': // 수정일 경우
				old_rec.inventory += trans_rec.inventory; // 내용물을 합해주고
				change_cnt++; // 변경 카운터를 1 올려줍니다.
				Get_Transaction(); // 새 트랜젝션을 가져옵니다
				break;
			case 'D': // 삭제일경우
				Get_Old(); // oldmaster 파일에서 가져온 다음
				delete_cnt++; // 삭제 카운터를 하나 올려주고
				Get_Transaction(); // 새 트랜젝션을 가져옵니다( 뉴 마스터에는 쓰지 않음)
				break;
			default:
				Error_Print('E');
				error_cnt++;
		...
		else // old_rec.id > trans_rec.id 의 경우
		{
			...
			{
			case 'A': // 추가일 경우에
				Set_New_Record(); // 새로운 레코드를 생성하고
				Get_Transaction(); // 트랜젝션을 하나 가져옵니다.
